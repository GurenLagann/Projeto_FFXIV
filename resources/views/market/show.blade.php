...
@section('title', 'Análise #' . $analysis->id . ' — FFXIV Market')
...
<div class="mb-7 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
    <div>
        <p class="ff-label mb-1" style="font-size:0.55rem;letter-spacing:0.2em;color:#2e3a60;">✦ &nbsp; RELATÓRIO DE MISSÃO</p>
        <div class="flex items-center gap-3 flex-wrap">
            <h1 class="ff-title text-2xl">Análise #{{ $analysis->id }}</h1>
    ...
    <div class="flex items-center gap-3 flex-shrink-0">
        ...
        <a href="{{ route('market.history') }}" class="ff-btn-ghost" style="font-size:0.6rem;">
            ◄ Histórico
        </a>
    </div>
</div>
...
<div class="grid grid-cols-3 gap-3 mb-7">
    ...
    <div class="ff-box p-5" style="border-color:#3a3060;">
        ...
        <p class="ff-num font-bold truncate" style="font-size:1.75rem;color:#f0c030;">
            {{ number_format(collect($analysis->results ?? [])->max('profit') ?? 0) }}
...
<div class="ff-box overflow-hidden">
    <div class="flex items-center gap-3 px-5 py-3.5" style="border-bottom:1px solid #252560;">
        <span style="color:#4040a0;font-size:0.6rem;" aria-hidden="true">◆</span>
        <h2 class="ff-label" style="font-size:0.62rem;color:#8899cc;margin:0;">Detalhes da Análise</h2>
    </div>
    ...
    @else
        <div class="overflow-x-auto">
            <table class="ff-table w-full" aria-label="Resultados da análise #{{ $analysis->id }}">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($analysis->results as $i => $r)
                        @php $profitable = $r['profit'] > 0; @endphp
                        <tr>
                            <td class="ff-num" style="color:#2e3a60;font-size:0.7rem;">{{ $i + 1 }}</td>
