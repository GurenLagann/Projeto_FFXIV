...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <p class="mt-1" style="font-size:0.78rem;color:#4a5470;">
...
<div class="ff-box overflow-hidden">
    @if($alerts->isEmpty())
        <div class="p-14 text-center">
            ...
            <p class="ff-label" style="color:#3a4060;font-size:0.65rem;">Nenhum alerta configurado</p>
                    ...
                    <tr class="group" x-data>
                        <td class="font-medium max-w-[200px]">
                            <div class="flex items-center gap-2"
                                ...
                                @if($alert->item?->icon)
                                    <img src="https://xivapi.com{{ $alert->item->icon }}"
                        ...
                        <td class="hidden sm:table-cell" style="color:#4a5470;font-size:0.8rem;">
                            {{ $alert->server->name ?? 'N/A' }}
                        </td>
                        <td class="text-right whitespace-nowrap">
                            <span class="ff-num" style="color:#f0c030;font-weight:600;">{{ number_format($alert->min_profit) }}</span>
                            <span style="font-family:'Share Tech Mono',monospace;font-size:0.65rem;color:#4a3a18;"> g</span>
                        </td>
                        <td class="text-right hidden md:table-cell ff-num" style="color:#4a5470;font-size:0.8rem;">
                            {{ $alert->min_margin }}%
                        </td>
                        <td class="text-right hidden lg:table-cell ff-num" style="color:#3a4060;font-size:0.72rem;">
                        ...
                        <td class="text-right">
                            <form action="{{ route('alerts.toggle', $alert) }}" method="POST" class="inline">
                                ...
                                <button type="submit"
                                        ...
                                        class="{{ $alert->is_active ? 'ff-badge-profit' : 'ff-badge-neutral' }}"
                                        style="cursor:pointer;border:none;background:none;padding:0.15rem 0.5rem;font-family:'Share Tech Mono',monospace;font-size:0.7rem;">
                        ...
                        <td class="text-right">
                            <form action="{{ route('alerts.destroy', $alert) }}" method="POST" class="inline"
                                ...
                                <button type="submit"
                                        ...
                                        class="opacity-0 group-hover:opacity-100 ff-label transition-opacity"
                                        style="font-size:0.6rem;color:#ff5533;background:none;border:none;cursor:pointer;transition:opacity 120ms;">
                                    Excluir
                                </button>
                            </form>
                        </td>
